...
class NET(object):
    ...
    def __init__(self, is_training=True):
        ...
        # Parameters #
        ##############
        self.image_size = cfg.IMAGE_SIZE # fixed size of images (224x224)
        self.output_size = int(cfg.NC) # amount of categories
    ...
    def build_network(self,
        ...
        with tf.variable_scope(scope):
            with slim.arg_scope( [slim.conv2d, slim.fully_connected],
                ...
                # 96 filters - size 11x11 - stride [4 4] - padding VALID
                print(net.shape)
                net = slim.max_pool2d(net, [3, 3], 2, padding='VALID', scope='pool_1') 
                # size 3x3 - stride [2 2] - padding VALID (default)
                ...
                # conv2 - pool2 - LRN
                print('conv2 - pool2 - LRN:')
                net = slim.conv2d(net, 256, 5, 1, activation_fn=tf.nn.relu, scope='conv_2') 
                # 256 filters - size 5x5 - stride [1 1] - padding SAME (default)
                print(net.shape)
                ...
                # 384 filters - size 3x3 - stride [1 1] - padding SAME (default)
                print(net.shape)
                ...
                # 256 filters - size 3x3 - stride [1 1] - padding SAME (default)
                print(net.shape)
                ...
                # FC-6 - Dropout
                print('FC-6 - Dropout:')
                net = slim.fully_connected(net, 4096, activation_fn=tf.nn.relu, scope='fc_6')
                net = tf.nn.dropout(net, self.keep_prob, name='dropout_6')
                print(net.shape)

                # FC-7 - Dropout
                print('FC-7 - Dropout:')
                net = slim.fully_connected(net, 4096, activation_fn=tf.nn.relu, scope='fc_7')
